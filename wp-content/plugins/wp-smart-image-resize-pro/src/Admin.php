...
if ( ! class_exists( '\WP_Smart_Image_Resize\Settings' ) ) :
    class Admin
    {
        ...
        function settings_field_enable_trim()
        {
            ...
            <label for="wp-sir-enable-trim">
                <input type="checkbox"
                       ...
                       <span class="hidden" id="wp-sir-trim-feather-wrap">
          Border size (px) <span class="woocommerce-help-tip"></span> <input type="number" name="wp_sir_settings[trim_feather]" style="width:70px" value="<?php echo $settings['trim_feather'] ?>">
          <span style="font-size:12px;color:#666" class="description">(This will leave a untouched "border" around image while trimming).</span>
        </span>
        ...
        function settings_field_enable_webp()
        {
            ...
            <p class="description">
                <?php _e(
                    "WebP reduces image file size by up to 90% comparing to PNG images without losing quality. Fallback to standard formats for browsers that can't display WebP images.",
                    WP_SIR_NAME
                ); ?>
            </p>
            <?php
