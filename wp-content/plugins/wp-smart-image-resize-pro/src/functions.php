...
if ( ! function_exists( 'wp_sir_is_processable' ) ) {
    ...
    function _wp_sir_is_processable( $attachment_id )
    {
        ...
        $post_ids = $wpdb->get_col( $wpdb->prepare( $sql, array_merge( [ $attachment_id ], $processable_post_types ) ) );
        ...
        if ( wp_sir_is_attachment_upload() ) {
            ...
            if ( empty( $post_type ) ) {
                foreach ( $processable_post_types as $referer_post_type ) {
                    if ( \WP_Smart_Image_Resize\Utilities\Request::is_referer( 'post_type=' . $referer_post_type ) ) {
                        return true;
                    }
                }
            }
